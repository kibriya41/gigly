  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-serif font-semibold text-[#1a3c34] dark:text-[#e8f4f0] tracking-tight">Active Projects</h1>
          <p className="text-[#5a7a72] dark:text-[#9fb3c8] mt-1.5 text-[15px]">Tasks where your proposal was accepted.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        {[
          { title: "In Progress", value: activeCount, icon: Clock, iconBg: "bg-[#eaf5f2] dark:bg-[#1a2435]", iconColor: "text-[#2a9d8f]", cardBg: "bg-white dark:bg-[#131c2b]" },
          { title: "Completed", value: completedCount, icon: CheckCircle2, iconBg: "bg-emerald-50", iconColor: "text-emerald-600", cardBg: "bg-white dark:bg-[#131c2b]" },
          { title: "Total Earned", value: `$${totalEarned.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, icon: DollarSign, iconBg: "bg-[#e2f1ed] dark:bg-[#1a2435]", iconColor: "text-[#1a3c34] dark:text-[#e8f4f0]", cardBg: "bg-[#f0f7f4] dark:bg-[#1a2435]" },
        ].map((stat, i) => {
          const Icon = stat.icon;
          return (
            <div key={i} className={`p-6 rounded-2xl border border-[#d4ebe6] dark:border-[#1e293b]/50 shadow-sm flex flex-col justify-between h-[140px] transition-all hover:shadow-md ${stat.cardBg}`}>
      ...
      {loading ? (
        <div className="flex items-center justify-center min-h-[30vh]">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-[#8aa89e] dark:text-[#6b7e94] text-sm font-medium">Loading your projects...</p>
          </div>
        </div>
      ) : error ? (
        ...
        <div className="bg-white dark:bg-[#131c2b] rounded-3xl border border-[#d4ebe6] dark:border-[#1e293b]/40 p-12 text-center max-w-xl mx-auto shadow-sm space-y-6">
          ...
          <div>
            <h3 className="text-xl font-bold text-[#1a3c34] dark:text-[#e8f4f0]">No Projects Yet</h3>
        ...
        <div className="space-y-6">
          {myProjects.map((project) => {
            ...
            return (
              <div
                key={project._id}
                className="bg-white dark:bg-[#131c2b] rounded-3xl border border-[#d4ebe6] dark:border-[#1e293b]/40 p-6 md:p-8 hover:shadow-md hover:border-[#2a9d8f]/30 transition-all space-y-4"
              >
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                  <div className="space-y-2 flex-1">
                    ...
                    <Link href={`/tasks/${task._id || ""}`} className="font-bold text-xl text-[#1a3c34] dark:text-[#e8f4f0] hover:text-[#2a9d8f] hover:underline transition-colors block">
                      {task.title || project.taskTitle || "Untitled Task"}
                    </Link>
                    <p className="text-sm text-[#5a7a72] dark:text-[#9fb3c8] leading-relaxed line-clamp-2">{task.description || "No description available."}</p>
                  </div>
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    ...
                    <span className="text-xs text-[#8aa89e] dark:text-[#6b7e94] font-medium">Agreed amount</span>
                  </div>
                </div>

                {/* Show deliverable link if completed */}
                {!isInProgress && project.deliverableUrl && (
                  <div className="bg-[#f4f8f6] dark:bg-[#1a2435] border border-[#d4ebe6] dark:border-[#1e293b] p-4 rounded-2xl flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2 min-w-0">
                      <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                      <span className="text-[#5a7a72] dark:text-[#9fb3c8] shrink-0">Deliverable URL:</span>
                    ...
                    <a href={project.deliverableUrl} target="_blank" rel="noopener noreferrer" className="text-[#2a9d8f] hover:text-[#1a3c34] dark:text-[#e8f4f0] p-1 shrink-0">
                ...
                <div className="mt-6 pt-4 border-t border-gray-100 flex items-center justify-between gap-4 flex-wrap">
                  <div className="flex flex-wrap items-center gap-4 text-xs text-[#8aa89e] dark:text-[#6b7e94] font-medium">
                    <span className="flex items-center gap-1.5">
                      <User className="w-4 h-4 text-[#2a9d8f]" />
                      Client: <span className="text-[#1a3c34] dark:text-[#e8f4f0] font-bold ml-1">{task.buyerEmail || "—"}</span>
                    </span>
                    <span className="flex items-center gap-1.5">
                      <Calendar className="w-4 h-4 text-[#2a9d8f]" />
                      Deadline: <span className="text-[#1a3c34] dark:text-[#e8f4f0] font-bold ml-1">
          ...
          <div className="bg-white dark:bg-[#131c2b] w-full max-w-md rounded-3xl shadow-2xl border border-[#d4ebe6] dark:border-[#1e293b]/50 overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            <div className="px-6 py-5 border-b border-[#d4ebe6] dark:border-[#1e293b]/30 flex items-center justify-between bg-[#f4f8f6] dark:bg-[#1a2435]">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-[#1a3c34] dark:text-[#e8f4f0]">Submit Deliverable</h3>
                  <p className="text-[10px] text-[#8aa89e] dark:text-[#6b7e94] mt-0.5">Complete and deliver task work</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmitDeliverable} className="p-6 space-y-4">
              ...
              <div className="bg-[#f0f9f6] dark:bg-[#1a2435] border border-[#d4ebe6] dark:border-[#1e293b] p-3 rounded-xl text-[11px] text-[#5a7a72] dark:text-[#9fb3c8] leading-relaxed">
              ...
              <div className="pt-2 flex items-center justify-end gap-3">
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-[#5a7a72] dark:text-[#9fb3c8] hover:bg-gray-50 transition-colors"
                >
                  Cancel
                </button>
                <button
