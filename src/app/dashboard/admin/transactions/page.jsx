...
export default function AdminTransactionsPage() {
  ...
  const stats = [
    { title: "Total Transactions", value: totalTransactions, icon: Receipt, iconBg: "bg-[#eaf5f2]", iconColor: "text-[#2a9d8f]" },
    { title: "Total Revenue (USD)", value: `$${totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, icon: DollarSign, iconBg: "bg-[#e2f1ed]", iconColor: "text-[#1a3c34]" },
  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto pb-16">
      {/* Header */}
      <div>
        <h1 className="text-4xl font-serif font-semibold text-[#1a3c34] tracking-tight">Transactions History</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl border border-[#d4ebe6]/40 p-12 text-center max-w-xl mx-auto shadow-sm space-y-6">
          ...
          <div>
            <h3 className="text-xl font-bold text-[#1a3c34]">No Transactions Found</h3>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-[#f4f8f6] border-b border-[#d4ebe6]/60">
                  <th className="text-left text-[10px] font-bold uppercase tracking-wider text-[#8aa89e] px-6 py-4">Task</th>
                  <th className="text-left text-[10px] font-bold uppercase tracking-wider text-[#8aa89e] px-6 py-4">Client</th>
                  <th className="text-left text-[10px] font-bold uppercase tracking-wider text-[#8aa89e] px-6 py-4">Freelancer</th>
                  <th className="text-left text-[10px] font-bold uppercase tracking-wider text-[#8aa89e] px-6 py-4">Amount</th>
                  <th className="text-left text-[10px] font-bold uppercase tracking-wider text-[#8aa89e] px-6 py-4">Date</th>
                  <th className="text-left text-[10px] font-bold uppercase tracking-wider text-[#8aa89e] px-6 py-4">Status</th>
                  <th className="text-left text-[10px] font-bold uppercase tracking-wider text-[#8aa89e] px-6 py-4">Transaction ID</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {filteredPayments.map((p) => (
                  <tr key={p._id} className="hover:bg-[#f4f8f6]/40 transition-colors">
                    <td className="px-6 py-4 text-sm text-[#1a3c34] font-semibold max-w-[200px] truncate">
                      {p.taskTitle || p.taskId}
                    </td>
                    <td className="px-6 py-4 text-sm text-[#5a7a72]">{p.clientEmail}</td>
                    <td className="px-6 py-4 text-sm text-[#5a7a72]">{p.freelancerEmail}</td>
                    ...
                    <td className="px-6 py-4 text-sm text-[#8aa89e]">
                    ...
                    <td className="px-6 py-4 text-xs text-[#8aa89e] font-mono">{p.transactionId}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <div className="bg-[#f4f8f6] border-t border-[#d4ebe6]/60 px-6 py-4 flex items-center justify-between flex-wrap gap-3">
            <span className="text-sm font-bold text-[#5a7a72]">
              Showing {filteredPayments.length} of {payments.length} transactions
            </span>
            <span className="text-sm font-extrabold text-[#1a3c34]">
              <span className="text-[#8aa89e] font-semibold text-xs mr-1">Total:</span>
