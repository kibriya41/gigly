    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh] max-w-md mx-auto text-center px-4">
        <div className="bg-white dark:bg-[#131c2b] p-8 rounded-3xl border border-[#d4ebe6] dark:border-[#1e293b]/50 shadow-md space-y-6">
          ...
          <p className="text-[#5a7a72] dark:text-[#9fb3c8]">Please sign in to view the admin dashboard.</p>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-bold uppercase tracking-widest text-[#2a9d8f] bg-[#eaf5f2] dark:bg-[#1a2435] px-3 py-1 rounded-full border border-[#d4ebe6] dark:border-[#1e293b]">
        ...
        <button
          onClick={fetchStats}
          disabled={loading}
          className="flex items-center gap-2 bg-[#f4f8f6] dark:bg-[#1a2435] hover:bg-[#eaf5f2] dark:bg-[#1a2435] border border-[#d4ebe6] dark:border-[#1e293b] text-[#1a3c34] dark:text-[#e8f4f0] px-5 py-2.5 rounded-full text-xs font-semibold transition-all w-fit disabled:opacity-50"
        >
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {statCards.map((stat, i) => {
              ...
              return (
                <div key={i} className={`p-6 rounded-2xl border border-[#d4ebe6] dark:border-[#1e293b]/50 shadow-sm flex flex-col justify-between h-[160px] transition-all hover:shadow-md ${stat.cardBg}`}>
                  ...
                  <div className="space-y-1 mt-4">
                    ...
                    {stat.sub && <p className="text-[11px] text-[#8aa89e] dark:text-[#6b7e94] font-medium">{stat.sub}</p>}
                  </div>
                </div>
              );
            })}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {insightCards.map((card, i) => {
              ...
              return (
                <div key={i} className="bg-white dark:bg-[#131c2b] rounded-2xl border border-[#d4ebe6] dark:border-[#1e293b]/40 p-5 space-y-2 shadow-sm">
                  <div className="flex items-center gap-2">
                    <Icon className="w-4 h-4 text-[#2a9d8f]" />
                    <span className="text-[10px] font-bold uppercase tracking-wider text-[#8aa89e] dark:text-[#6b7e94]">{card.label}</span>
                  </div>
                  <p className="text-xl font-bold text-[#1a3c34] dark:text-[#e8f4f0]">
                    {loading ? <span className="inline-block w-12 h-5 bg-gray-100 animate-pulse rounded" /> : card.value}
                  </p>
                  <p className="text-[10px] text-[#8aa89e] dark:text-[#6b7e94] font-medium">{card.sub}</p>
                </div>
              );
            })}
          </div>
          ...
          {/* Quick Actions */}
          <div className="bg-white dark:bg-[#131c2b] rounded-3xl border border-[#d4ebe6] dark:border-[#1e293b]/40 shadow-sm p-6 md:p-8 space-y-5">
            <div>
              <h2 className="text-xl font-serif font-semibold text-[#1a3c34] dark:text-[#e8f4f0]">Quick Actions</h2>
              <p className="text-[#8aa89e] dark:text-[#6b7e94] text-sm mt-0.5">Jump straight to management areas.</p>
            </div>
              ...
              {quickLinks.map((link) => {
                ...
                return (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="group flex items-center justify-between gap-3 p-5 rounded-2xl border border-[#d4ebe6] dark:border-[#1e293b]/60 hover:border-[#2a9d8f]/40 hover:bg-[#f0f9f6] dark:bg-[#1a2435]/30 transition-all"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-[#eaf5f2] dark:bg-[#1a2435] flex items-center justify-center shrink-0">
                        <Icon className="w-5 h-5 text-[#2a9d8f]" />
                      </div>
                      <div>
                        <h3 className="font-bold text-sm text-[#1a3c34] dark:text-[#e8f4f0] group-hover:text-[#2a9d8f] transition-colors">{link.label}</h3>
                        <p className="text-[11px] text-[#8aa89e] dark:text-[#6b7e94]">{link.desc}</p>
                      </div>
                    </div>
                    <ArrowRight className="w-4 h-4 text-[#8aa89e] dark:text-[#6b7e94] group-hover:text-[#2a9d8f] transition-colors" />
                  </Link>
                );
              })}
            </div>
