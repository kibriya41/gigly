...
export default function AdminTasksPage() {
  ...
  const stats = [
    { title: "Total Tasks", value: totalTasks, icon: Briefcase, iconBg: "bg-[#eaf5f2]", iconColor: "text-[#2a9d8f]" },
  ...
  const statusStyle = (status) => {
    ...
    if (s === "in progress") return "text-[#2a9d8f] bg-[#eaf5f2] border-[#d4ebe6]";
  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto pb-16 relative">
      ...
      {/* Header */}
      <div>
        <h1 className="text-4xl font-serif font-semibold text-[#1a3c34] tracking-tight">Manage Tasks</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl border border-[#d4ebe6]/40 p-12 text-center max-w-xl mx-auto shadow-sm space-y-6">
          ...
          <div>
            ...
            <p className="text-[#8aa89e] text-sm mt-1.5">No tasks match your current filters.</p>
          </div>
        </div>
      ) : (
        <div className="space-y-4">
          {filteredTasks.map((task) => {
            ...
            return (
              <div key={task._id} className="bg-white rounded-3xl border border-[#d4ebe6]/40 p-6 hover:shadow-md transition-all">
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                  <div className="space-y-2 flex-1">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-[#eaf5f2] text-[#2a9d8f] border border-[#d4ebe6]/50">
                    ...
                    <Link href={`/tasks/${task._id}`} className="font-bold text-lg text-[#1a3c34] hover:text-[#2a9d8f] transition-colors block">
                      {task.title}
                    </Link>
                    <p className="text-sm text-[#5a7a72] line-clamp-2">{task.description}</p>
                    <div className="flex flex-wrap items-center gap-4 text-xs text-[#8aa89e] font-medium pt-1">
                      <span>Client: <span className="text-[#1a3c34] font-bold">{task.buyerEmail}</span></span>
                      <span className="flex items-center gap-1">
                        <DollarSign className="w-3.5 h-3.5 text-[#2a9d8f]" />
                        <span className="text-[#1a3c34] font-bold">${Number(task.budget || 0).toLocaleString()}</span>
                      </span>
                      {task.deadline && (
                        <span className="flex items-center gap-1">
                          <Calendar className="w-3.5 h-3.5 text-[#2a9d8f]" />
                          <span className="text-[#1a3c34] font-bold">{new Date(task.deadline).toLocaleDateString()}</span>
                        </span>
                      )}
                    </div>
                  </div>
          ...
          <div className="bg-[#f4f8f6] rounded-2xl border border-[#d4ebe6]/60 px-6 py-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/45 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl border border-red-100 p-6 space-y-6 animate-in zoom-in-95 duration-150">
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <button onClick={() => setDeletingTaskId(null)} className="px-5 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-[#5a7a72] hover:bg-gray-50">
                Cancel
              </button>
